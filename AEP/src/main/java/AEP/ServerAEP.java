...
public class ServerAEP {
				...
				while (!clientCommand.equals("exit")) {
					...
					case "?":
						toClient.println("?                    --> Show all commands \n"
								...
								+ "sendFile <filepath> --> Copy a file to the seted path \n" 
								+ "exit --> Exit" + "\n" 
								+ "$");
						break;
					case "path":
					...
					case "showFiles":
						...
						break;
					case "showAll":
						showSubDirectories(toClient, path);
						toClient.println("$");
						break;
					case "createDir":
	...
	public void showSubDirectories(PrintWriter toClient, String path) {
		try {
			File root = new File(path);

			showSubDirectoriesRecursive(root, "", toClient);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void showSubDirectoriesRecursive(File input, String indentation, PrintWriter toClient) {
		toClient.println(indentation + input.getAbsolutePath());
		if (input.isDirectory()) {
			for (File file : input.listFiles())
				showSubDirectoriesRecursive(file, indentation + " ", toClient);
		}
	}
